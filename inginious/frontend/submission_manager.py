...
class WebAppSubmissionManager:
    ...
    def get_feedback_from_submission(self, task, submission, only_feedback=False, show_everything=False, translation=gettext.NullTranslations()):
        """
            Get the input of a submission. If only_input is False, returns the full submissions with a dictionnary object at the key "input".
            Else, returns only the dictionnary.
        ...
        if "problems" in submission:
            ...
            for pid in submission["problems"]:
                ...
                if isinstance(submission["problems"][pid], str):  # fallback for old-style submissions
                    submission["problems"][pid] = (submission.get('result', 'crash'), problem.prepare_feedback(
                        submission["problems"][pid],
                        show_everything,
                        translation
                    ) if problem else submission["problems"][pid])
                else:  # new-style submission
                    submission["problems"][pid] = (submission["problems"][pid][0], problem.prepare_feedback(
                        submission["problems"][pid][1],
                        show_everything,
                        translation
                    ) if problem else submission["problems"][pid])
        return submission
